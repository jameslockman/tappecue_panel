	...
	req.addEventListener('load', function () {
		...
		if (responseCode === 200) {
			if (responseVals.length > 0) {
				...
				sessionMessage = "<h4>Session: <select id='sessionSelector' class='sesecticker'></select></h4>";
			...
			for (var i = 1; i <= Object.keys(smokeData).length; i++) {
				...
				if (smokeData[i].active === "1") {
					if (Number(smokeData[i].current_temp) > (Number(smokeData[i].max_temp) + warningThreshold)) {
						$("#probe" + i).removeClass("probe-warning probe-inactive probe-ok probe-danger probe-cold");
...
function balanceProbes() {
	...
	if ($("#sessionMessage").attr("aria-expanded") === "true") {
...
function smokeIcon(data) {
	...
	if (data.chamber === "1") {
		if (data.name==="Cruise Set Points") {
			if (data.current_temp === "32 ") {
				return ' <img src="../img/status_icons/fan_off.png" class="smokeIcon" alt="Tappecue Cruise Control Fan"/>'
			} else {
				return ' <img src="../img/status_icons/fan_on.png" class="smokeIcon" alt="Tappecue Cruise Control Fan"/>'
			}
