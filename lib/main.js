...
$(document).ready(function () {
    ...
    $("#timeZone").selectpicker('val','US/Eastern');
    balanceProbes();
    console.log("Ready!");
});
...
function getTimeZone(apiToken) {
    ...
    req.addEventListener('load', function () {
        ...
        if (responseCode === 200) {
            ...
            console.log("Unable to get Time Zone: "+responseCode);
        }
    });
...
function signout() {
    ...
    if ($("[name='rememberUsername']").bootstrapSwitch('state')) {
        console.log("OK to store username!");
...
function getToken() {
    ...
    fs.readFile(path.join(getStoragePath(), "tappecuedata"), function (err, data) {
        if (err) {
            console.log("getToken: No token set: " + err);
            var tappecueSession = {};
            tappecueSession.username = "";
            tappecueSession.pw = "";
